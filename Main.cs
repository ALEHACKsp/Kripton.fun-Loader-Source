    ...
    public partial class Main : Form
    {
        ...
        private void OnlineLists()
        {
            try
            {
                ...
                while (dr.Read())
                {
                    lstOnline.Items.Add(dr["username"].ToString());
                }
                db.Dispose();
                db.Close();
            }
        ...
        private void AllMessageLists()
        {
            while (true)
            {
                try
                {
                    ...
                    while (dr.Read())
                    {
                        txtmommys.Text += "(" + dr["time"].ToString() + " / " + dr["msg"].ToString() +"\n\n";
                    }
                    txtmommys.SelectionStart = txtmommys.TextLength;
                    txtmommys.ScrollToCaret();
                    db.Dispose();
                    db.Close();
                    OnlineLists();
                    Thread.Sleep(2000);

...
namespace RealTime_Chat
{
    public partial class Register : Form
    {
        ...
        {
            Create(txtUsername.Text, txtPassword.Text, txtEmail.Text, txtSecretanswer.Text);
        }
        #region RegisterForm
        private void Create(String username, String password, String email, String secretanswer)
        {
             ...
             txtSecretanswer.Text.Trim() != "")
            {
                ...
                if (pingStatus.Status == IPStatus.Success)
                {
                    try
                    {
                        db.Close();
                        db.Open();
                        cmd = new MySqlCommand("Insert Into user(username,password,email,secretanswer,status) Values ('"
                           + username.Trim() + "','"
                           + password.Trim() + "','"
                           + email.Trim() + "','"
                           //+ fullname.Trim() + "','"
